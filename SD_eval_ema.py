...
def load_model_from_config(config, ckpt, use_ema=True):
...
class StableDiffusionEvaluator:
    def __init__(self, config_path, model_path, dataset_path, batch_size=10, num_samples=50000, 
                image_size=512, ddim_steps=50, ddim_eta=1.0, 
        ...
        self.model, self.global_step = load_model_from_config(self.config, self.model_path, use_ema=self.use_ema)
    ...
    def evaluate(self):
        ...
        print(f"Using {'EMA weights' if self.use_ema else 'regular weights'} for sampling")
...
def main():
    ...
    parser.add_argument('--use_ema', action='store_true', default=True, help='Use EMA weights for sampling')
