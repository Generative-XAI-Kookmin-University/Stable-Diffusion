...
class StableDiffusionEvaluator:
    def __init__(self, config_path, model_path, dataset_path, batch_size=10, num_samples=50000, image_size=512, ddim_steps=50, ddim_eta=1.0, device='cuda' if torch.cuda.is_available() else 'cpu', save_generated_images=False, samples_dir='./generated_samples'):
        ...
        self.compute_real_statistics()
        self.load_real_features_raw()
    
    def compute_real_statistics(self):
        ...
        feature_sq_sum = None
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dataloader, desc="Processing real images for statistics")):
                ...
                if count == 0:
                    feature_sum = np.zeros(features.shape[1])
                    feature_sq_sum = np.zeros((features.shape[1], features.shape[1]))
                feature_sum += np.sum(features, axis=0)
                for j in range(batch_size):
                    feature_sq_sum += np.outer(features[j], features[j])
                count += batch_size
                if count >= n_samples_needed:
                    break
        self.real_mu = feature_sum / count
        self.real_sigma = feature_sq_sum / count - np.outer(self.real_mu, self.real_mu)
        print(f"Processed {count} real images")
    
    def load_real_features_raw(self):
        dataset = EvalDataset(self.dataset_path, self.image_size)
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=False, num_workers=4, drop_last=False)
        n_samples_needed = min(self.num_samples, len(dataset))
        n_batches_needed = int(np.ceil(n_samples_needed / self.batch_size))
        features_list = []
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dataloader, desc="Loading real features")):
                if i >= n_batches_needed:
                    break
                batch = batch.to(self.device)
                batch = 2 * batch - 1
        ...
        print(f"Loaded {self.real_features.shape[0]} real features")
    ...
    def compute_generated_statistics(self):
        ...
        with torch.no_grad():
            for i in tqdm(range(n_rounds), desc="Generating samples and computing statistics"):
                current_batch_size = min(self.batch_size, self.num_samples - count)
                if current_batch_size <= 0:
                    break
                shape = [current_batch_size, self.model.model.diffusion_model.in_channels, self.model.model.diffusion_model.image_size, self.model.model.diffusion_model.image_size]
                ...
                if self.save_generated_images:
                    for j in range(current_batch_size):
                        if count + j < 100:
                            ...
                            img.save(os.path.join(self.samples_dir, f'sample_{count+j:05d}.png'))
                features = self.get_inception_features(x_samples)
                fake_features_list.append(features.cpu())
                probs = torch.nn.functional.softmax(features, dim=1).cpu().numpy()
                all_probs.append(probs)
                features_np = features.cpu().numpy()
                if count == 0:
                    feature_sum = np.zeros(features_np.shape[1])
                    feature_sq_sum = np.zeros((features_np.shape[1], features_np.shape[1]))
                feature_sum += np.sum(features_np, axis=0)
                for j in range(current_batch_size):
                    feature_sq_sum += np.outer(features_np[j], features_np[j])
                count += current_batch_size
                del samples, x_samples, features
                torch.cuda.empty_cache()
        fake_mu = feature_sum / count
        fake_sigma = feature_sq_sum / count - np.outer(fake_mu, fake_mu)
        all_probs = np.concatenate(all_probs, axis=0)
        is_score = self.calculate_inception_score_from_probs(all_probs)
        fid = calculate_frechet_distance(fake_mu, fake_sigma, self.real_mu, self.real_sigma)
        fake_features = torch.cat(fake_features_list, dim=0)
        return is_score, fid, fake_features
    
    def calculate_inception_score_from_probs(self, probs, splits=10):
        scores = []
        subset_size = probs.shape[0] // splits
        for k in range(splits):
            subset = probs[k * subset_size: (k + 1) * subset_size]
            p_y = np.mean(subset, axis=0)
            kl_d = subset * (np.log(subset + 1e-10) - np.log(p_y + 1e-10))
    ...
    def evaluate(self):
        ...
        is_score, fid, fake_features = self.compute_generated_statistics()
